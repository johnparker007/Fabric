...
uint8_t configuration_stage, flash_mode, load_saw_flash;
...
#ifndef FAKE_AMBER_OMIT_FLASH
PRODUCTION_EXPORT void SetFlashROMMode(uint8_t value) { flash_mode = value; }
#endif
PRODUCTION_EXPORT uint32_t LoadROM(uint8_t *a, uint8_t *b, uint8_t *, uint8_t *) {
  load_saw_flash = flash_mode;
  return initialised && a && (flash_mode || b) ? (b ? 2u : 1u) : 0u;
}
...
PRODUCTION_EXPORT void SetReelExt(uint8_t) { ++configuration_calls; }
...
PRODUCTION_EXPORT uint32_t GetOutputSnapshot(void *buffer, uint32_t size) {
  ...
  s.MatrixLamps[30].Brightness = static_cast<float>(load_saw_flash);
  s.MatrixLamps[31].Brightness = static_cast<float>(cycles);
  ...
  s.AlphaDotDisplayCount = 1;
  s.ElectronicMechCount = 1; s.MeterCount = 6; s.DipCount = 16; s.HopperCount = 2;
  s.LedCount = 512;
