...
int main() {
  ...
  auto unknown = request(FAKE_PRODUCTION_AMBER_PATH);
  std::strcpy(unknown.machine_identifier, "unknown-amber");
  CHECK(FabricCreateSession(runtime, &unknown, &session) != FABRIC_OK);
  auto mpu_missing_coin = request(FAKE_PRODUCTION_AMBER_MPU5_MISSING_COIN_IN_PATH);
  std::strcpy(mpu_missing_coin.machine_identifier, "barcrest-mpu5");
  CHECK(FabricCreateSession(runtime, &mpu_missing_coin, &session) == FABRIC_NOT_SUPPORTED);
  CHECK(error(runtime).find("required export 'CoinIn'") != std::string::npos);
  CHECK(error(runtime).find("barcrest-mpu5") != std::string::npos || error(runtime).find(FAKE_PRODUCTION_AMBER_MPU5_MISSING_COIN_IN_PATH) != std::string::npos);
  const char *mpu_program[] = {"mpu5-program.rom"};
  auto mpu = request(FAKE_PRODUCTION_AMBER_MPU5_PATH);
  std::strcpy(mpu.machine_identifier, "barcrest-mpu5");
  mpu.rom_paths = mpu_program;
  mpu.rom_path_count = 1;
  mpu.machine_configuration = &c;
  mpu.machine_configuration_size = sizeof(c);
  CHECK(FabricCreateSession(runtime, &mpu, &session) == FABRIC_OK);
  CHECK(FabricSessionInitialise(session) == FABRIC_OK);
  FabricLamp mpu_lamps[320]{};
  FabricReel mpu_reels[8]{};
  FabricCharacterDisplay mpu_chars[2]{};
  FabricSegmentDisplay mpu_segments[40]{};
  FabricMachineSnapshot mpu_snap{};
  mpu_snap.struct_size = sizeof(mpu_snap);
  mpu_snap.struct_version = FABRIC_ABI_VERSION_CURRENT;
  mpu_snap.lamps = mpu_lamps;
  mpu_snap.lamp_capacity = 320;
  mpu_snap.reels = mpu_reels;
  mpu_snap.reel_capacity = 8;
  mpu_snap.character_displays = mpu_chars;
  mpu_snap.character_display_capacity = 2;
  mpu_snap.segment_displays = mpu_segments;
  mpu_snap.segment_display_capacity = 40;
  CHECK(FabricSessionGetSnapshot(session, &mpu_snap) == FABRIC_OK);
  CHECK(mpu_snap.lamp_count == 320 && mpu_snap.character_display_count == 2 && mpu_snap.segment_display_count == 40);
  CHECK(mpu_chars[0].characters[0] == 0x1111 && mpu_chars[0].attributes[0] == 1);
  CHECK(mpu_chars[1].characters[0] == 0x2222 && mpu_chars[1].attributes[0] == 2);
  CHECK(mpu_segments[39].segment_masks[0] == (0x80u | 39u));
  CHECK(FabricSessionAdvance(session, 1000000) == FABRIC_OK);
  CHECK(FabricSessionGetSnapshot(session, &mpu_snap) == FABRIC_OK);
  CHECK(mpu_reels[0].position == 16000);
  FabricInput mpu_input{};
  mpu_input.struct_size = sizeof(mpu_input);
  mpu_input.struct_version = FABRIC_ABI_VERSION_CURRENT;
  mpu_input.kind = FABRIC_INPUT_COIN; mpu_input.coin_channel = 5; mpu_input.coin_value = 7; mpu_input.active = 1;
  CHECK(FabricSessionSubmitInput(session, &mpu_input) == FABRIC_OK);
  CHECK(FabricSessionGetSnapshot(session, &mpu_snap) == FABRIC_OK);
  CHECK(mpu_lamps[3].brightness == 57.0f);
  mpu_input.active = 0; CHECK(FabricSessionSubmitInput(session, &mpu_input) == FABRIC_OK);
  mpu_input.active = 1; mpu_input.coin_value = 11;
  CHECK(FabricSessionSubmitInput(session, &mpu_input) == FABRIC_INPUT_REJECTED);
  mpu_input.kind = FABRIC_INPUT_DIGITAL; mpu_input.numerical_index = 250; mpu_input.active = 1;
  CHECK(FabricSessionSubmitInput(session, &mpu_input) == FABRIC_OK);
  CHECK(FabricSessionGetSnapshot(session, &mpu_snap) == FABRIC_BACKEND_ERROR);
  mpu_input.active = 0; CHECK(FabricSessionSubmitInput(session, &mpu_input) == FABRIC_OK);
  mpu_input.numerical_index = 251; mpu_input.active = 1; CHECK(FabricSessionSubmitInput(session, &mpu_input) == FABRIC_OK);
  CHECK(FabricSessionGetSnapshot(session, &mpu_snap) == FABRIC_BACKEND_ERROR);
  mpu_input.active = 0; CHECK(FabricSessionSubmitInput(session, &mpu_input) == FABRIC_OK);
  CHECK(FabricSessionReset(session) == FABRIC_OK);
  CHECK(FabricSessionGetSnapshot(session, &mpu_snap) == FABRIC_OK);
  CHECK(mpu_lamps[4].brightness == 7.0f);
  FabricAudioFormat mpu_format{}; mpu_format.struct_size = sizeof(mpu_format); mpu_format.struct_version = FABRIC_ABI_VERSION_CURRENT;
  CHECK(FabricSessionGetAudioFormat(session, &mpu_format) == FABRIC_OK);
  CHECK(mpu_format.sample_rate == 48000 && mpu_format.channel_count == 2);
  int16_t mpu_audio[96]{}; uint32_t mpu_written = 0;
  CHECK(FabricSessionAdvance(session, 1000000) == FABRIC_OK);
  CHECK(FabricSessionReadAudio(session, mpu_audio, 48, &mpu_written) == FABRIC_OK);
  CHECK(mpu_written == 48 && mpu_audio[0] == 200);
  CHECK(FabricSessionShutdown(session) == FABRIC_OK);
  FabricDestroySession(session);
  auto mpu_reset_fails = request(FAKE_PRODUCTION_AMBER_MPU5_RESET_FAILS_PATH);
  std::strcpy(mpu_reset_fails.machine_identifier, "barcrest-mpu5");
  mpu_reset_fails.rom_paths = mpu_program; mpu_reset_fails.rom_path_count = 1;
  CHECK(FabricCreateSession(runtime, &mpu_reset_fails, &session) == FABRIC_OK);
  CHECK(FabricSessionInitialise(session) == FABRIC_BACKEND_ERROR);
  CHECK(session_error(session).find("Reset failed") != std::string::npos);
  FabricDestroySession(session);
